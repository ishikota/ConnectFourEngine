...
class Heuristic():
    ...
    def solveSTMH(self, board):
        ...
        for i in range(board.WIDTH-5+1):
            ...
            # find pattern 2
            elif p[i+2] == '-' and p[i+1] != '-' and p[i+1] == p[i+3]:
                player = p[i+1]
    ...
    def calcHeuristicValue(self, klm,kclm,kcn,kot,ket,clm,cclm,ccn,cot,cet):
        ...
        killed_critical_line_num = killed_even_threat_num if self.ME == 'O' else killed_odd_threat_num
        create_critical_line_num = create_odd_threat_num if self.ME == 'O' else create_even_threat_num
    ...
    def checkWinningLine(self, board, player, row, col):
        ...
        for d in range(7):
            ...
            for k in range(board.CONNECT_K-1):
                ...
                if ni==0 or (board.table[ni][nj] == '-' and board.table[ni-1][nj] != '-'):
                    continue
                # if this square is threat, then check its even-odd and remember it.
                if board.table[ni][nj] == '-':
            ...
            if d%2==1 or d==6:
                ...
                if line_nums[d/2] >= board.CONNECT_K:
                    ...
                    if threat_even_odd[d/2] == 1:
                        ...
                        even_threat_num += 1
                    if (player == 'O' and threat_even_odd[d/2] == 1) or\
                             (player == 'X' and threat_even_odd[d/2] == 2):
